...
- **定型入力（ひな形）** — 毎回同じ値（通貨・区分など）は固定し、日付・金額など変わる値だけを打つ。時刻の足し引きや「○時以前なら有」の判定、チェックボックスのオン・オフ、複数日分の一覧からの入力にも対応
...
経費精算のように「半分は毎回同じ値、残りだけが変わる」画面向けです。ひな形を JSON ファイルに書き、設定（歯車）の **ひな形ファイルを読み込む** から取り込みます。「定型入力」タブでひな形を選び、変わる値だけを入れて **この画面に入力** を押すと、「貼り付けて入力」と同じ流れ（記憶済み → 名前一致 → AI）で欄に割り当て、そのまま入力まで進めます（AIの確信度が低い欄は入れません）。
...
- **チェックボックス**: 値を `"✓"` にすると、同じ名前のチェックボックスをオンにします（`"無"` `"いいえ"` などならオフ）。チェックボックスは名前が完全に一致したときだけ入れ、AIには推測させません
- **一覧（複数日分）**: セットに `"list": { "date": ["対象日"], "times": ["出勤時刻", "退勤時刻"] }` を書くと、「定型入力」タブに一覧欄が出ます。`2/3(火) 出勤 8:47 退勤 21:27` のような行を貼り、行を選ぶ（または **次の行 ▶**）と、その行の最初の日付と時刻（出てきた順）が欄に入ります。年の無い日付は今日に近い年として扱います
...
node test/serve.mjs         # http://localhost:5188 に模擬画面（/ 経費、/profile 会員登録、/kintai 勤怠、/panel サイドパネル）
